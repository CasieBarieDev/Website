document.querySelectorAll('[data-spigotid]').forEach((element) => {
    ...
    fetch(`https://api.spiget.org/v2/resources/${spigotId}/versions/latest`).then(response => response.json()).then(data => {
        const versionElement = element.querySelector('.version');
        if(versionElement) {versionElement.textContent = `v${data.name}⠀`;}
    });

    fetch(`https://api.spiget.org/v2/resources/${spigotId}`).then(response => response.json()).then(data => {
...
document.querySelectorAll('[data-libraryname]').forEach((element) => {
    ...
    fetch(`https://api.github.com/repos/${libraryName}/releases/latest`).then(response => response.json()).then(data => {
        const versionElement = element.querySelector('.version');
